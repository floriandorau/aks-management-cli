...
const assertContext = (name, resourceGroup, subscription) => {
    ...
    if (!resourceGroup) {
        throw Error(
            'No "resourceGroup" given. Check "kubectl config current-context"'
        );
    }
    if (!subscription) {
        throw Error(
            'No subscription given. Use option --subscription or try to set active subscription'
        );
    }
};
...
const addIp = async (ip, context) => {
    ...
    authorizedIpRanges = authorizedIpRanges.filter(
        (range) => range !== DEFAULT_AUTHORIZED_IP_RANGE
    );

    if (context.authorizedIp) {
        console.log(
            `Removing previously authorized ip '${context.authorizedIp}' from authorized ip-ranges`
        );
        const previousAuthorizedIpCidr = toCidrNotation(context.authorizedIp);
        authorizedIpRanges = authorizedIpRanges.filter(
            (range) => range !== previousAuthorizedIpCidr
        );
    }
...
const removeIp = async (ip, context) => {
    ...
    const authorizedIpRanges = remoteAuthorizedIpRanges.filter(
        (range) => range !== ipCidr
    );
    if (authorizedIpRanges.length === 0) {
        authorizedIpRanges.push(DEFAULT_AUTHORIZED_IP_RANGE);
        console.log(
            `No authorized ip-range left. Will set default authorized ip-range for security reasons [${DEFAULT_AUTHORIZED_IP_RANGE}] `
        );
    }
